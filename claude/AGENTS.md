...
- **Speak in Simplified Technical English (STE), the ASD-STE100 Standard — in chat, not only in files.** The written-artifact rule below (comments, docs, commits, PR bodies) is the same rule; this line extends it to what you say to Drew and to what you write into an agent brief. Active voice. One instruction per sentence. A word carries one meaning for the whole session, and a term already used never gets a synonym. Procedural sentence: 20 words. Descriptive sentence: 25. At most three nouns in a row. A long sentence that needs a re-read has already failed, however correct it is.
