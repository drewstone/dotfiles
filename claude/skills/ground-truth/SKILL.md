...
4. **Trace your own run** — `~/code/traces analyze --harness claude-code --last 1` (or the repo's failure-mode analyst) EARLY, not after the user is furious. It catches what you can't see from inside: status-without-a-moved-number, re-measure churn, a fake baseline.
