...
Fires on: plateau ≤2% across ≥2 `/evolve` cycles · "this approach isn't working" · "think bigger". Use `/evolve` for "make this metric go up"; `/breakout` when the target itself is the cage; `/polish` when correctness is fine and the gap is rubric-driven.

## Flow

1. **Read state before code.** `.agent/current.json`, `.agent/progress.md`, newest `.agent/pursuits/*.md`, tail `.agent/experiments.jsonl`. Another skill active <24h → reconcile or `/governor`; never open a 2nd generation in flight.
...
4. **Run a tournament, ≥2 architectures.** Pull the ranked field from `/hypothesize` when one exists. Argue them against each other, pick by predicted leverage, graft from the runners-up. State one moonshot (10×, not 10%) as adopted or rejected with a reason.
...
8. **Diff-audit before Evaluate.** `/critical-audit --diff-only` (fallback `bash ${SKILL_DIR}/diff-audit.sh`). Fix every CRITICAL and HIGH; skip only for a 1-file reversible change.
...
| Rule | Why (measured: n=3,671 pursuit artifacts, n=760 `/pursue` run rows) |
|---|---|
...
| **≥2 architectures in contention**, losers recorded with the reason they lost. | 785/3,671 (21%) recorded any alternative; a 1-candidate pursuit is `/evolve` in costume. |
...
| Plateau evidence | <k> `/evolve` cycles, best Δ +<x>%, <run ids / paths> |
...
|---|---|---|
| Δ ≥ +5% on the plateau metric and it is still moving | `/evolve` | new baseline + the ≤3 tunable knobs left |
| Δ < +2% and this is pursue cycle ≥3 on the same metric | `/breakout` | the mechanism behind the ceiling + all 3 Δs |
| Tournament field <2 architectures at step 4 | `/hypothesize` | the binding constraint + candidates already rejected |
| ≥2 surviving architectures share 0 files | `/multi-pursue` | 1 track spec each + the shared harness command |
| Δ ≤ 0 and n<3 per side, or the CI spans 0 | `/autopsy` | raw per-run rows + the exact harness command |
| Δ ≤ 0 at n≥3 but the design ran below its assumed scale | `/dont-collapse-the-architecture` | the regime the thesis assumed vs the one tested |
| Diff audit CRIT+HIGH >0 unfixed | `/critical-audit` | `--diff-only` + the branch name |
| Verdict ADVANCE and ≥1 rubric dimension failing | `/polish` | the rubric + the failing dimensions |
| Subjective plateau metric with 0 judges | `/eval-agent` | the product-value claim + 10 rated examples |
...
267/760 past rows (35%) logged no verdict and 746/760 (98%) no `--next`, so `/governor` could not chain them. Both flags are required; the row is provenance, not evidence.
